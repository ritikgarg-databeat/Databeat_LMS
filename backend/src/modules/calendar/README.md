...
defines internal domain shapes, and `calendar.validation.ts` holds the express-validator chains
...
affected users, and database constraints enforce a valid target shape. The complete audience list
is returned only to Super Admin or the event creator, so shared events do not expose unrelated
cohort names to another trainer or trainee.
