...
Mounted _inside_ `groups.routes.ts` at `/:id/members` (`Router({ mergeParams: true })`), not as a
...
Roster mutations and CSV import require an active, non-deleted group. Trainers may operate only
on their assigned group; Super Admins may operate on any active group. Archived groups grant no
course/assessment/AI/progress access until explicitly restored.
