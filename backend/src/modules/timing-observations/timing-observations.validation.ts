...
import {
  MAX_TIMING_OBSERVATION_DURATION_SECONDS,
  MAX_TIMING_OBSERVATION_NOTES_LENGTH,
} from '@/constants/timing-observations';
...
const lessonIdFilter = query('lessonId')
...
const trainerIdFilter = query('trainerId')
  .optional({ values: 'falsy' })
  .isUUID()
  .withMessage(VALIDATION_MESSAGES.INVALID_ID('trainerId'));
...
export const timingObservationsValidation = {
  create: [
    ...
    body('manualDurationSeconds')
      ...
      .withMessage(
        `manualDurationSeconds must be between 1 and ${MAX_TIMING_OBSERVATION_DURATION_SECONDS} seconds.`,
      )
      .toInt(),
    body('aiAssistedDurationSeconds')
      .isInt({ min: 1, max: MAX_TIMING_OBSERVATION_DURATION_SECONDS })
      .withMessage(
        `aiAssistedDurationSeconds must be between 1 and ${MAX_TIMING_OBSERVATION_DURATION_SECONDS} seconds.`,
      )
      .toInt(),
    body('notes')
      .optional({ values: 'falsy' })
