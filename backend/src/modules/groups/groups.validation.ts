...
const descriptionChain = body('description').optional({ values: 'falsy' }).isString().isLength({ max: MAX_GROUP_DESCRIPTION_LENGTH });
...
const capacityChain = body('capacity')
  ...
  .toInt();
const dateChain = (field: string) => body(field).optional({ values: 'falsy' }).isISO8601().withMessage(`${field} must be a valid date.`);
