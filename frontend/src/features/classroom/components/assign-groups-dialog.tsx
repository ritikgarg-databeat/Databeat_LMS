...
import { useAssignGroupMutation, useCourseAssignmentsQuery, useUnassignGroupMutation } from '../hooks';
...
function AssignGroupsDialog({ courseId, open, onOpenChange }: AssignGroupsDialogProps) {
  const [unassigning, setUnassigning] = useState<CourseGroupAssignmentSummary | null>(null);

  const { data: assignments, isLoading: isLoadingAssignments } = useCourseAssignmentsQuery(open ? courseId : undefined);
